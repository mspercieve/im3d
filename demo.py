...
d_model = int(rel_cfg.d_g/4)   # 64/4 = 16
...
def run(cfg):
    ...
    # print('Time elapsed (w/ loader): %s.' % (end - start1))
    ...
    cam_R_out = get_rotation_matix_result(cfg.bins_tensor,
                                          ...
                                          torch.argmax(est_data['roll_cls_result'], 1), est_data['roll_reg_result'])   # [3, 3]
    ...
    # save results
    nyu40class_ids = [int(evaluate_bdb['classid']) for evaluate_bdb in bdb3D_out_form_cpu]  # list [10]
    save_path = cfg.config['demo_path'].replace('inputs', 'outputs')
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    # save layout
    savemat(os.path.join(save_path, 'layout.mat'),
            mdict={'layout': lo_bdb3D_out[0, :, :].cpu().numpy()})   # [8, 3]
    ...
    savemat(os.path.join(save_path, 'r_ex.mat'),
            mdict={'cam_R': cam_R_out[0, :, :].cpu().numpy()})   # [3, 3]
    # save meshes
    current_faces = est_data['out_faces'][interval[0]:interval[1]]   # list [10]  ->  [#, 3]
    current_coordinates = est_data['meshes'][interval[0]:interval[1]] # list [10] ->  [3, #]
    ...
    pre_layout_data = sio.loadmat(os.path.join(save_path, 'layout.mat'))['layout']   # [8, 3]
    ...
    cam_K = np.loadtxt(os.path.join(cfg.config['demo_path'], 'cam_K.txt'))     # [3, 3]
