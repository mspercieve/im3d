...
class Config(object):
    def __init__(self, dataset):
        ...
        if self.dataset == 'sunrgbd':
            self.metadata_path = os.path.join(ROOT, 'data/sunrgbd')
    ...
    def __initiate_bins(self):
        ...
        if self.dataset == 'sunrgbd':
            ...
            bin['layout_centroid_avg'] = layout_avg_dict['layout_centroid_avg']     # [2.41730 , 2.62259, 1.7938]
            bin['layout_coeffs_avg'] = layout_avg_dict['layout_coeffs_avg']     # [1.61612, 1.33176, -0.0435]

            '''layout orientation bin'''
            NUM_LAYOUT_ORI_BIN = 2
            ORI_LAYOUT_BIN_WIDTH = np.pi / 4
            bin['layout_ori_bin'] = [[np.pi / 4 + i * ORI_LAYOUT_BIN_WIDTH, np.pi / 4 + (i + 1) * ORI_LAYOUT_BIN_WIDTH] for i in range(NUM_LAYOUT_ORI_BIN)]   # [[pi/4, pi/2], [pi/2, 3pi/4]]
