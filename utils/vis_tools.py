...
# nyu_colorbox = np.array([[int(c, 16) / 255. for c in (rgb[1:3], rgb[3:5], rgb[5:7])] for rgb in objectColors])
colorbox_path = 'external/cooperative_scene_parsing/evaluation/vis/nyu_colorbox.mat'
#colorbox_path = '../external/cooperative_scene_parsing/evaluation/vis/nyu_colorbox.mat'
if not os.path.exists(colorbox_path):
    np.random.seed(0)
    init_colorbox = np.array(sns.hls_palette(n_colors=len(NYU40CLASSES), l=.45, s=1.))
    vis_class = [
        "bed", "books", "bookshelf", "box", "cabinet", "chair", "door", "dresser",
        "lamp", "mirror", "night_stand", "picture", "pillow", "shelves", "sofa",
        "table", "television"
    ]
    vis_index = [i for i in range(len(NYU40CLASSES)) if NYU40CLASSES[i] in vis_class]
    vis_color_indexs = [int(i * len(NYU40CLASSES) / len(vis_class)) for i in range(len(vis_class))]
    vis_color = init_colorbox[vis_color_indexs]
    np.random.shuffle(vis_color)
    nvis_color_indexs = [i for i in range(len(NYU40CLASSES)) if i not in vis_color_indexs]
    nvis_color = init_colorbox[nvis_color_indexs]
    nyu_colorbox = []
    ivis_color = 0
    invis_color = 0
    for i in range(len(NYU40CLASSES)):
        if i in vis_index:
            nyu_colorbox.append(vis_color[ivis_color])
            ivis_color += 1
        else:
            nyu_colorbox.append(nvis_color[invis_color])
            invis_color += 1
    nyu_colorbox = np.stack(nyu_colorbox)
    sio.savemat(colorbox_path, {'nyu_colorbox': nyu_colorbox})
else:
    nyu_colorbox = sio.loadmat(colorbox_path)['nyu_colorbox']
