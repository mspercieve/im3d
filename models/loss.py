...
class JointLoss(BaseLoss):
    def __call__(self, est_data, gt_data, bins_tensor, layout_results, train):
        ...
        # layout cls pred loss
        if 'lo_mask_cls_score' in est_data.keys() and train:
            cls_loss = cls_criterion(est_data['lo_mask_cls_score'], gt_data['size_cls'])
            return {'phy_loss': phy_loss, 'bdb2D_loss': bdb2D_loss, 'corner_loss': corner_loss, 'lo_cls_pred_loss': cls_loss}, \
