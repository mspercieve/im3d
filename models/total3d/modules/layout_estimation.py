...
class PoseNet(nn.Module):
    def __init__(self, cfg, optim_spec=None):
        ...
        '''Modules'''
        self.resnet = resnet.resnet34(pretrained=False, feat = True)
    ...
    def forward(self, x):
        x, feat = self.resnet(x)      # (b, 3, 256, 256) -> (2, 2048)

        # branch for camera parameters
        cam = self.fc_1(x)
        cam_feat = self.relu_1(cam)
        cam = self.dropout_1(cam_feat)
        cam = self.fc_2(cam)
        ...
        lo = self.fc_layout(x)
        lo_feat = self.relu_1(lo)
        lo = self.dropout_1(lo_feat)
        ...
        return pitch_reg, roll_reg, pitch_cls, roll_cls, lo_ori_reg, lo_ori_cls, lo_centroid, lo_coeffs, x, feat, (cam_feat, lo_feat)