...
class TOTAL3D(BaseNetwork):
    ...
    def forward(self, data):
        ...
        if self.cfg.config[self.cfg.config['mode']]['phase'] in ['layout_estimation', 'joint']:
            ...
            lo_centroid_result, lo_coeffs_result, a_features, res_features, fc_features = self.layout_estimation(data['image'])
            ...
            layout_output = {'pitch_reg_result':pitch_reg_result, 'roll_reg_result':roll_reg_result,
                             ...
                             'lo_afeatures': a_features, 'lo_res_features': res_features, 'lo_fc_features': fc_features}
        ...
        if self.cfg.config[self.cfg.config['mode']]['phase'] in ['object_detection', 'joint']:
            ...
            r_features, a_r_features, fc_features = self.object_detection(data['patch'], data['size_cls'], data['g_features'],
            ...
            object_output = {'size_reg_result':size_reg_result, 'ori_reg_result':ori_reg_result,
                             ...
                             'odn_afeature': a_features, 'odn_rfeatures': r_features, 'odn_arfeatures': a_r_features, 'odn_fc_features': fc_features}
        ...
        if self.cfg.config[self.cfg.config['mode']]['phase'] in ['joint']:
            ...
            if hasattr(self, 'output_adjust'):
                ...
                input['bdb2D_pos_resize'] = data['bdb2D_pos_resize']
                input['K'] = data['K']
    ...
    def loss(self, est_data, gt_data, train=True):
        '''
        ...
        if self.cfg.config[self.cfg.config['mode']]['phase'] in ['joint']:
            joint_loss, extra_results = self.joint_loss(est_data, gt_data, self.cfg.bins_tensor, layout_results, train)
            ...
            total_joint_loss = sum(joint_loss_weighted.values())
            ...
            total_joint_loss_unweighted = sum([v.detach() for v in joint_loss.values()])
        ...
        if self.cfg.config[self.cfg.config['mode']]['phase'] == 'joint':
            ...
            total3d_loss = total_object_loss + total_joint_loss + total_layout_loss
            total3d_loss_unweighted = total_object_loss_unweighted + total_joint_loss_unweighted \
                                      + total_layout_loss_unweighted
            # return {'total':total3d_loss, **layout_loss, **object_loss, **mesh_loss, **joint_loss,
            #         'total_unweighted': total3d_loss_unweighted}
            return {'total': total3d_loss, **layout_loss, **object_loss, **joint_loss,
                    'total_unweighted': total3d_loss_unweighted}
        else:
            raise NotImplementedError