...
class GCNN2(nn.Module):
    ...
    def forward(self, output):
        ...
        res_feat1, res_feat2, res_feat3 = output['lo_res_features']
        ...
        split = output['split']
        for i in range(lo_fused.shape[0]):
            if i ==0:
                lo_cat = lo_fused[i].unsqueeze(0).expand([split[i][1], -1])
            else:
                lo_cat = torch.cat((lo_cat, lo_fused[i].unsqueeze(0).expand([split[i][1]-split[i][0], -1])))

        # branch to predict the size
        size = self.fc1(torch.cat((size_feat, lo_cat), dim=1))
        size = self.relu_1(size)
        size = self.dropout_1(size)
        size = self.fc2(size)

        # branch to predict the orientation
        ori = self.fc3(torch.cat((ori_feat, lo_cat), dim=1))
        ori = self.relu_1(ori)
        ...
        # branch to predict the centroid
        centroid = self.fc5(torch.cat((centroid_feat, lo_cat), dim=1))
        ...
        # branch to predict the 2D offset
        offset = self.fc_off_1(torch.cat((offset_feat, lo_cat), dim=1))
