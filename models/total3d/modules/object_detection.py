...
class Bdb3DNet(nn.Module):
    ...
    def forward(self, x, size_cls, g_features, split, rel_pair_counts):
        ...
        size_feat = self.relu_1(size)
        size = self.dropout_1(size_feat)
        size = self.fc2(size)

        # branch to predict the orientation
        ori = self.fc3(a_r_features_cat)
        ori_feat = self.relu_1(ori)
        ori = self.dropout_1(ori_feat)
        ori = self.fc4(ori)
        ...
        centroid_feat = self.relu_1(centroid)
        centroid = self.dropout_1(centroid_feat)
        ...
        offset_feat = self.relu_1(offset)
        offset = self.dropout_1(offset_feat)
        offset = self.fc_off_2(offset)

        return size, ori_reg, ori_cls, centroid_reg, centroid_cls, offset, a_features, r_features, a_r_features, (size_feat, ori_feat, centroid_feat, offset_feat)