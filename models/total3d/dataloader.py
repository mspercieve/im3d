...
class Total3D_Dataset(SUNRGBD):
    ...
    def __getitem__(self, index):
        ...
        for i, bdb in enumerate(boxes['bdb2D_pos']):
            ...
            patch.append(img)
            bdb2D_pos_resize.append(np.array([int(bdb[0]/image.size[0]*WIDTH_PATCH), int(bdb[1]/image.size[1]*HEIGHT_PATCH), int(bdb[2]/image.size[0]*WIDTH_PATCH), int(bdb[3]/image.size[0]*HEIGHT_PATCH)]))

        boxes['bdb2D_pos_resize'] = np.array(bdb2D_pos_resize)
        ...
        if key == 'boxes_batch':
            ...
            for subkey in batch[0][key]:
                if subkey == 'mask':
                    ...
                    # list_of_tensor = []
                    # for elem in batch:
                    #     if subkey in elem[key]:
                    #         list_of_tensor += [recursive_convert_to_torch(elem[key][subkey])]
