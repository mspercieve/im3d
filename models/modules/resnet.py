...
#         self.layer_new = self._make_layer(block, 512, layers[3], stride=1)
#
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, input_channels=3, feat = False):
        self.feat = feat
        self.inplanes = 64
        super(ResNet, self).__init__()
    ...
    def forward(self, x):
        x = self.conv1(x)   # (2, 64, 128, 128)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)   #(2, 64, 64, 64)

        x = self.layer1(x) #(2, 64, 64, 64)
        x1 = self.layer2(x) #(2, 128, 32, 32)
        x2 = self.layer3(x1) #(2, 256, 16, 16)
        x3 = self.layer4(x2) #(2, 512, 8, 8)

        x = self.avgpool(x3)  #(2, 512, 2, 2)
