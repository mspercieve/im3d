...
def eval(cfg, tester, test_loader, model_name):
    '''
    train epochs for network
    :param cfg: configuration file
    :param tester: specific tester for networks
    ...
    start = time()
    with torch.no_grad():
        eval_loss_recoder = test_func(cfg, tester, test_loader)
    cfg.log_string('Test time elapsed: (%f).' % (time()-start))
    fid = open('./eval/%s.txt' %(model_name), 'w')
    for key, test_loss in eval_loss_recoder.items():
        fid.write('%15s : %f' %(key, test_loss.avg))
        if len(test_loss.cls) > 0:
            cfg.log_string({k: v.avg for k, v in test_loss.cls.items()})
